...
TEST_CASE("Infrastructure_UdpSender-SendAPacket") {
    ...
    std::this_thread::sleep_for(1ms);
    REQUIRE(sender->CheckIsStarted());
    auto buffer = new utility::StringSizedBuffer("Hello World!");
    std::promise<void> done_promise;
    auto done_future = done_promise.get_future();
    auto buffer_ptr = std::shared_ptr<utility::StringSizedBuffer>(
        buffer, [p = std::move(done_promise)](utility::StringSizedBuffer *ptr) mutable {
            p.set_value();
        }
    );
