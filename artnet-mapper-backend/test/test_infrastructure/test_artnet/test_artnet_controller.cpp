...
TEST_CASE("Infrastructure_ArtNet-SendPacket") {
    ...
    auto buffer = new utility::VectorSizedBuffer(50);
    std::iota (std::begin(buffer->_buffer), std::end(buffer->_buffer), 1);
    std::promise<void> done_promise;
    auto done_future = done_promise.get_future();
    auto buffer_ptr = std::shared_ptr<utility::VectorSizedBuffer>(
            buffer, [p = std::move(done_promise)](utility::VectorSizedBuffer *ptr) mutable {
                p.set_value();
            }
    );
