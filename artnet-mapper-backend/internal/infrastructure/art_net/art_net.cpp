...
namespace infrastructure {
    ...
    void ArtNet::Post(utility::SizedBufferPtr &&art_net_pixels) {
        if (!_work_started) {
            return;
        }
    ...
    void ArtNet::run(std::stop_token st) {
        ...
        while (!st.stop_requested()) {
            utility::SizedBufferPtr art_net_pixels;
            {
                std::unique_lock<std::mutex> lock(_work_mutex);
    ...
    void ArtNet::postPixels(utility::SizedBufferPtr &&art_net_pixels) {
        for (auto &[_, controller]: _controllers) {
